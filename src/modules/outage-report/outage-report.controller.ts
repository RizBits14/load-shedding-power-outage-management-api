...
export const createOutageReport = async (
    ...
    try {
        const result =
            createOutageReportSchema.safeParse(
                req.body,
            );
        ...
        if (!profile) {
            return res.status(400).json({
                success: false,
                message:
                    "Create your customer profile before reporting an outage",
            });
        }
        ...
        if (profile.area.deletedAt) {
            return res.status(400).json({
                success: false,
                message:
                    "Your assigned area is currently unavailable",
            });
        }
        ...
        if (existingReport) {
            return res.status(409).json({
                success: false,
                message:
                    "You already have an unresolved outage report",
            });
        }
        ...
        return res.status(201).json({
            success: true,
            message:
                "Outage report submitted successfully",
            data: report,
        });
    } catch (error) {
        console.error(
            "Create outage report error:",
            error,
        );
...
export const getMyOutageReports = async (
    ...
    try {
        ...
        return res.status(200).json({
            success: true,
            message:
                "Your outage reports retrieved successfully",
            data: reports,
        });
    } catch (error) {
        console.error(
            "Get customer outage reports error:",
            error,
        );
...
export const getMyOutageReportById = async (
    ...
    try {
        ...
        return res.status(200).json({
            success: true,
            message:
                "Outage report retrieved successfully",
            data: report,
        });
    } catch (error) {
        console.error(
            "Get outage report error:",
            error,
        );
...
export const getOutageReports = async (
    ...
    try {
        const result =
            outageReportQuerySchema.safeParse(
                req.query,
            );
        ...
        return res.status(200).json({
            success: true,
            message:
                "Outage reports retrieved successfully",
            data: reports,
...
export const getOutageReportById = async (
    ...
    try {
        ...
        const report =
            await prisma.outageReport.findUnique({
                where: {
                    id,
                },
...
export const reviewOutageReport = async (
    ...
    try {
        ...
        const result =
            reviewOutageReportSchema.safeParse(
                req.body,
            );
        ...
        if (report.status !== "PENDING") {
            return res.status(400).json({
                success: false,
                message:
                    "Only pending outage reports can be reviewed",
            });
        }
        ...
        await createNotificationSafely({
            ...
            title:
                updatedReport.status ===
                    "VERIFIED"
                    ? "Outage report verified"
                    : "Outage report rejected",

            message:
                updatedReport.status ===
                    "VERIFIED"
                    ? "Your outage report has been verified by the PowerSync operations team."
                    : `Your outage report was rejected.${updatedReport.reviewNote
                        ? ` Reason: ${updatedReport.reviewNote}`
                        : ""
                    }`,

            entityType: "OUTAGE_REPORT",
